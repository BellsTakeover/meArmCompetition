"""
mearm_guided_transfer.py

Shared meArm script for 3 separate Raspberry Pis / computers.
Each person chooses ARM_ID 1, 2, or 3, calibrates 3 coordinates,
and then runs an automatic repeating motion:

    rest -> intake -> outtake -> restnnn

This version intentionally ignores the syringe pump for now.
It is built around the same style as the user's earlier meArm code:
- meArm.meArm(...)
- arm.move_to(x, y, z)
- arm.get_position()
...
2. Guided calibration window:
   - manually move the arm with keyboard
   - save 3 points: rest, intake, outtake
   - save to JSON

3. Safe workspace constraints:
   - clamp x/y/z into a limited zone per arm
   - coarse radial reach check

4. Automatic run mode:
   - loops through rest/intake/outtake/rest
   - records timing for each move
   - saves timings to CSV

Suggested workflow
------------------
1. On each Pi:
   python3 mearm_guided_transfer.py --arm-id arm1 --mode calibrate
   python3 mearm_guided_transfer.py --arm-id arm2 --mode calibrate
   python3 mearm_guided_transfer.py --arm-id arm3 --mode calibrate

2. After saving points, test one cycle:
   python3 mearm_guided_transfer.py --arm-id arm1 --mode run --cycles 1

3. Record timing averages:
   python3 mearm_guided_transfer.py --arm-id arm1 --mode run --cycles 10

Files created
-------------
- mearm_transfer_points.json   : saved coordinates for all arms
- mearm_move_log.csv           : timing log for every move
"""
...
POSITION_TOL_MM = 4.0
MOVE_TIMEOUT_S = 4.0
MOVE_SETTLE_S = 0.25 #change based on meArm stability
KEY_INTERVAL_S = 0.03
WINDOW_SIZE = (780, 520)
DEFAULT_STEP_MM = 5.0
MIN_STEP_MM = 1.0
MAX_STEP_MM = 20.0 #set a plan for user ability to increment change
...
# These are starter-safe zones. Tune with real testing.
...
class TimedMeArm:
    def __init__(self, arm_id: str, address: int = DEFAULT_ADDRESS):
        ...
        self.arm = meArm.meArm(address=address, logger=self.logger)
    ...
    def move_to(self, target: Pose, settle_s: float = MOVE_SETTLE_S) -> Dict[str, float]:
        ...
        return {
            ...
            "reached": float(1 if reached else 0),
...
# -----------------------------------------------------------------------------
def draw_text(screen, font, small_font, arm_id: str, task: str, pose_name: str,
              edit_pose: Pose, current_pose: Pose, saved_points: Dict[str, Dict[str, float]],
              step_mm: float, status: str) -> None:
...
def calibration_mode(controller: TimedMeArm, points_path: str) -> None:
    ...
    while running:
        ...
        if now - last_key_time >= KEY_INTERVAL_S:
            ...
            # Keep the same style as the user's earlier controller:
        ...
        current_pose = controller.current_pose()
        draw_text(screen, font, small_font, controller.arm_id, ARM_TASKS[controller.arm_id],
                  current_edit_name, edit_pose, current_pose, saved, step_mm, status)
        clock.tick(60)
...
# Automatic run mode
# -----------------------------------------------------------------------------
def run_mode(controller: TimedMeArm, points_path: str, cycles: int) -> None:
    points = load_points(points_path)
    ...
    # First home to rest so the timing sequence is consistent.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--arm-id", required=True, choices=["arm1", "arm2", "arm3"],
                        help="Which arm this Pi is controlling")
    parser.add_argument("--mode", default="calibrate", choices=["calibrate", "run"],
    ...
    parser.add_argument("--points-file", default=POINTS_FILE,
...
def main() -> int:
    ...
    if args.mode == "calibrate":
        calibration_mode(controller, args.points_file)
    elif args.mode == "run":
        run_mode(controller, args.points_file, args.cycles)
