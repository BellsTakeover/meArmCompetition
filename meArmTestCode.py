...
Important note:
- This script calibrates in x/y/z bucket positions, not raw base/elbow/wrist angles
- This version is adapted to the OLDER meArm.py API on the Raspberry Pi:
    arm = meArm.meArm()
    arm.begin(address=0x6F)
    arm.gotoPoint(x, y, z)
    arm.getPos()
...
DEFAULT_POINTS_FILE = "mearm_transfer_points.json"
DEFAULT_LOG_FILE = "mearm_move_log.csv"
...
# -----------------------------------------------------------------------------
# For Thonny, just change these and press Run.
ARM_ID = "arm1"          # "arm1", "arm2", or "arm3"
...
# Starter-safe workspaces. Tune after testing.
...
class TimedMeArm:
    def __init__(self, arm_id: str, address: int = DEFAULT_ADDRESS):
        ...
        # OLD meArm.py API on your Pi
        self.arm = meArm.meArm()
        self.arm.begin(address=address)

    def current_pose(self) -> Pose:
        x, y, z = self.arm.getPos()
    ...
    def move_to(self, target: Pose, settle_s: float = MOVE_SETTLE_S) -> Dict[str, float]:
        ...
        # Old API returns True/False
        reached = bool(self.arm.gotoPoint(safe_target.x, safe_target.y, safe_target.z))
        ...
        reached = reached and self.at_pose(safe_target)
...
def load_points(path: str = DEFAULT_POINTS_FILE) -> Dict[str, Dict[str, Dict[str, float]]]:
...
def save_points(data: Dict[str, Dict[str, Dict[str, float]]], path: str = DEFAULT_POINTS_FILE) -> None:
...
# Logging helpers
# -----------------------------------------------------------------------------
def append_move_log(row: Dict[str, object], path: str = DEFAULT_LOG_FILE) -> None:
...
def log_move_event(controller: TimedMeArm, info: Dict[str, float], cycle: int, segment: str, path: str = DEFAULT_LOG_FILE) -> None:
    row = {
        ...
        if now - last_key_time >= KEY_INTERVAL_S:
            ...
            if moved:
                try:
                    ...
                    info = controller.move_to(new_pose, settle_s=0.02)
                    edit_pose = new_pose
                    status = f"Moved {current_edit_name.upper()} preview | {info['elapsed_s']:.3f}s"
